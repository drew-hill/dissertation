/* 
    V0.9
   Mar 5, 2017 by L. Drew Hill at UC Berkeley
   Updates: (from v0.8) active ping of the ADC
...
compile (in appriate directory) with:
  ...
  gcc -pthread -o mcp_volttest mcp_volttest.c -lpigpio -l rt RPi_SHT1x_pigpio.c -lm
...
once in the appropriate directory (probably ~/NestC) launch with screen, sudo, and nice to set priority to -20 (gives max priority to these processes):
  screen sudo nice -n -20 ./mcp_volttest
...
#define FILENAME "/home/lawson/NestFiles/logpm_temp_calibration_24an.txt"
...
void *ir_read(void *pString)
{
  ...
  for(;;)
  {
    ...
    if (gpioRead(ir_pin_in) ==1)
    {
      ...
      // turn off higher voltage
      gpioWrite(volt_pin_out, 0);
...
void main(void)
{
  ...
  gpioSetMode(ir_pin_in, PI_INPUT);
  gpioSetMode(blue_pin_in, PI_INPUT);
  gpioSetMode(volt_pin_out, PI_OUTPUT);
