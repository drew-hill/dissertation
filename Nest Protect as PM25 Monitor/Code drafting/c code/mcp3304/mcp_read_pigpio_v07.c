/* 
   ...
   Updates: (from v0.61) remove GPIO interrupt structure-- infinite loop structure w/9 seconds of sleep

send to RPi with
  scp -r /Users/lawsonhill/Box\ Sync/Nest\ -\ dissertation/NestRead lawson@192.168.29.100:~

compile (in appriate directory) with:
  gcc -pthread -o mcp_read_pigpio mcp_read_pigpio_v07.c -lpigpio -l rt RPi_SHT1x_pigpio.c -lm
...
#define FILENAME "/home/lawson/NestFiles/allc_test_infinitefor_Jan22.txt"
...
void *temp_rh_10sec(void *pString)
{
  ...
  for(;;)
  {
    ...
    /* sleep for ~ 10 seconds and then take another temp/rh reading */
    sleep(9);
    usleep(999999);
  }
  ...
  for(;;)
  {
   ...
   /* if blue in_pin goes high */
    if (gpioRead(blue_pin_in) ==1)
    {
      ...
      // Print results to terminal -- NOTE: 'get_temp' has an inherent \n after... must go at end
      printf("blue, %s, %.1f, %.1f, %.1f, %s", time_buff, adcValue, temp, rh, systemp);   

      // Print results to file -- NOTE: 'get_temp' has an inherent \n after... must go at end
      file_out = fopen(FILENAME, "a"); //open file to append
      fprintf( file_out, "blue, %s, %.1f, %.1f, %.1f, %s", time_buff, adcValue, temp, rh, systemp);  // print to file
      fclose( file_out );
...
void main(void)
{
  ...
  pthread_create(&thread2, NULL, ir_read,"ir read");
  pthread_create(&thread3, NULL, blue_read,"blue read");
