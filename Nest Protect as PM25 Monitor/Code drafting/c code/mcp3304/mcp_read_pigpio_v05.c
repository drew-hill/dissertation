/* 
   ...
   Updates: (from v0.4) alter to put blue and IR timing closer together and extend IR LED blink

send to RPi with
  scp -r /Users/lawsonhill/Box\ Sync/Nest\ -\ dissertation/NestRead lawson@192.168.29.100:~

compile (in appriate directory) with:
  gcc -pthread -o mcp_read_pigpio mcp_read_pigpio_v05.c -lpigpio -l rt RPi_SHT1x_pigpio.c -lm
   ...
   // while loop to detect edge rise, then perform functions
    for (;;)
    {
        ...
        if (gpioRead(ir_pin_in) ==1)
        {
            ...
            // read sample time immediately after ADC-- important to get Blue and IR timing very close together; rh and temp do not need as much temporal precision
            time (&timer);
            ...
            // Make LED blink for half of a second -- wait half, sleep half
