/* 
   ...
   Updates: (from v0.6) put temp/rh sensor read function in its own loop that re-writes global variable
...
#define FILENAME "/home/lawson/NestFiles/alec_drew_funtimes.txt"
...
void *temp_rh_2sec(void *pString, tempData *data)
{
...
void ir_read (int gpio, int level, unsigned int tick, tempData *userdata)
{
  time_t timer;
  ...
  printf("ir, %s, %.1f, %p, %f, %s", time_buff, adcValue, temp, rh, systemp);   

  // Print results to file -- NOTE: 'get_temp' has an inherent \n after... must go at end
  file_out = fopen(FILENAME, "a"); //open file to append
  fprintf( file_out, "ir, %s, %.1f, %p, %f, %s", time_buff, adcValue, temp, rh, systemp);  // print to file
  fclose( file_out );
...
void blue_read (int gpio, int level, unsigned int tick, tempData *userdata)
{
  time_t timer;
  char time_buff[26]; //buffer for time object
  struct tm* tm_info;
...
//todo: don't store this on the stack, use malloc
  ...
  gpioSetISRFuncEx( ir_pin_in, RISING_EDGE, -1, ir_read, &data );


  /* Set Interrup for BLue LED */
  gpioSetISRFuncEx( blue_pin_in, RISING_EDGE, -1, blue_read,  &data );

// (int gpio, int level, unsigned int tick, void *data)
 

  /* set up continuous looper to keep the program alive*/
  pthread_create(&thread1, NULL, looper,"Thread 1 - looper");
  pthread_create(&thread2, NULL, temp_rh_2sec, &data);
