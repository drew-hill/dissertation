/* 
   ...
   Updates: (from v0.6) put temp/rh sensor read function in its own loop that re-writes global variable, blue loop now writes temp and rh info, filter out bounce-based ADC readings, pulldown resistors

send to RPi with
  scp -r /Users/lawsonhill/Box\ Sync/Nest\ -\ dissertation/NestRead lawson@192.168.29.100:~

compile (in appriate directory) with:
  gcc -pthread -o mcp_read_pigpio mcp_read_pigpio_v061.c -lpigpio -l rt RPi_SHT1x_pigpio.c -lm
...
#define FILENAME "/home/lawson/NestFiles/allc_test_Jan22.txt"

float rh = 0;
float temp = 0;
char systemp[15] = "temp=-0C\n";
...
void *temp_rh_10sec(void *pString)
{
  ...
  for(;;)
  {
    ...
    /* sleep for 2 seconds and then take another temp/rh reading */
    sleep(10);
  }
...
void ir_read (int gpio, int level, unsigned int tick)
{
  ...
  printf("ir, %s, %.1f, %d, %.1f, %.1f, %s", time_buff, adcValue, tick, temp, rh, systemp);   

  // Print results to file -- NOTE: 'get_temp' has an inherent \n after... must go at end
  file_out = fopen(FILENAME, "a"); //open file to append
  fprintf( file_out, "ir, %s, %.1f, %d, %.1f, %.1f, %s", time_buff, adcValue, tick, temp, rh, systemp);  // print to file
  fclose( file_out );
...
void blue_read (int gpio, int level, unsigned int tick)
{
  ...
  // Print results to terminal -- NOTE: 'get_temp' has an inherent \n after... must go at end
  printf("blue, %s, %.1f, %d, %.1f, %.1f, %s", time_buff, adcValue, tick, temp, rh, systemp);   

  // Print results to file -- NOTE: 'get_temp' has an inherent \n after... must go at end
  file_out = fopen(FILENAME, "a"); //open file to append
  fprintf( file_out, "blue, %s, %.1f, %d, %.1f, %.1f, %s", time_buff, adcValue, tick, temp, rh, systemp);  // print to file
  fclose( file_out );
...
void main(void)
{
  ...
  /* Set Inputs as inputs with pulldown resistors*/
  gpioSetMode(ir_pin_in, PI_INPUT);
  gpioSetPullUpDown(ir_pin_in, PI_PUD_DOWN);
  gpioSetMode(blue_pin_in, PI_INPUT);
  gpioSetPullUpDown(blue_pin_in, PI_PUD_DOWN);
  ...
  pthread_create(&thread1, NULL, temp_rh_10sec,"tempthread");
