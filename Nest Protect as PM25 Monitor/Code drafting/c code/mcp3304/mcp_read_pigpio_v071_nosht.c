/* 
   ...
   Updates: (from v0.7) smart sleep, based on ADC value

send to RPi with
  scp -r /Users/lawsonhill/Box\ Sync/Nest\ -\ dissertation/NestRead lawson@192.168.29.100:~

compile (in appriate directory) with:
  gcc -pthread -o mcp_read_pigpio mcp_read_pigpio_v071_nosht.c -lpigpio -l rt RPi_SHT1x_pigpio.c -lm
...
void main(void)
{
  ...
  pthread_create(&thread1, NULL, blue_read,"tempthread");
  pthread_create(&thread2, NULL, ir_read,"ir read");
  // pthread_create(&thread3, NULL, temp_rh_10sec,"blue read");
