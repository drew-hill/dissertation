...
filename <- "~/Desktop/nest_datalog_data_ep_2016dec12.txt"
...
# device code 'epsilon'
dt[, ir := ir - (-0.53634 * tempC + 0.03731*rh) ]

# cal_epsilon_ir <- 551.47172 + (-0.53634 * tempC) + (0.03731*rh)
...
# low IR flag ... iterative
	ggplot(dt_proc, aes(x=datetime, y = ir_lag0) ) + geom_point()
	...
	ggplot(dt_step1, aes(x=datetime, y = ir_lag0) ) + geom_point()
...
ggplot(dtf) + geom_point(aes(x=datetime, y = ir_ra30),size=0.5 ) + geom_line(aes(x=datetime, y = ir_ra30)) + ylim(c(530,555))
