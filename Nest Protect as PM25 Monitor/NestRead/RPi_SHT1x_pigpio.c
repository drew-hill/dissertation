...
Modifications: shift GPIO library from the BCM2835 C library to the pigpio C library; decreased resolution to 8/12 bit; fits sensor V3 instead of V4
...
void SHT1x_InitPins( void ) 
{
	// SCK line as output but set to low first
		gpioWrite(RPI_GPIO_SHT1x_SCK, 0);
		gpioSetMode(RPI_GPIO_SHT1x_SCK, PI_OUTPUT);
	gpioWrite(RPI_GPIO_SHT1x_SCK, 0);

	// DATA to input. External pull up.
	// Set PORT to 0 => pull data line low by setting port as output
		gpioSetPullUpDown(RPI_GPIO_SHT1x_DATA, PI_PUD_OFF);
	gpioWrite(RPI_GPIO_SHT1x_DATA,0);
...
unsigned char SHT1x_Get_Measure_Value( unsigned short int * value ) 
{
	...
	while( SHT1x_GET_BIT )
	{
		gpioDelay(5000);			// $$$$$$$$$$$$$$$$$$ 1 ms not working $$$$$$$$$$$$$$$$$$$$$$$$
...
//----------------------------------------------------------------------------------------
{
	// const float C1=-4.0000;            		// for 8 Bit
	// const float C2=+0.6480;           		// for 8 Bit
	// const float C3=-0.00072000;      		// for 8 Bit

	// const float T1=+0.01;             			// for 8 Bit
	// const float T2=+0.00128;           		// for 8 Bit

	// const float	D1 = -39.66;			// For 3.3 Volt power supply, Centigrade
	// const float	D2 = 0.04;			// For 12 Bit temperature, Centigrade

	const float C1=-4.0000;            		// for 12 Bit
	const float C2=+0.0405;           		// for 12 Bit
	const float C3=-0.0000028000;      		// for 12 Bit
